...
class NegationContextAnalyzer(PackProcessor):
    ...
    def _process(self, input_pack: DataPack):
        r"""
        Negation Context analysis is done on the basis of tagging entities and
        detecting phrases. We use the negation rules' file to set all rule
        phrases which would convey a negative context depending on their
        positioning, i.e. before or after an entity.
        ...
        The first rule implies, if the phrase 'absence of' is present in the
        sentence before an entity, we annotate the entity as negation
        polarity=True. Similarly, if the phrase 'does not exist' occurs in the
        sentence after an entity, it implies negative context. We also handle
        conjunctions, based off of rules from the rules' file.
        For example, given a sentence:

        "CT scan shows xyz but absence of lesions."
        Our implementation would set negation polarity for 'xyz' as False, but
        won't carry it to 'lesions' because we detect 'but' as a conjunction in
        the sentence. Hence, we decide polarity for 'lesions' independently as
        True given the presence of 'absence' phrase before 'lesions'.

        All rule phrases are tagged in the sentence as [PREN]..[PREN],
        [POST]..[POST] or [CONJ]..[CONJ], depending on how they are defined in
        the rules' file. All entity mentions are first annotated with the
        [ENTITY]..[ENTITY] tags.
        On negation detection, we replace those with [NEGATED]..[NEGATED].
        These tags are then used to define NegationContext annotations in the
        `input_pack` with corresponding polarity.
        """
        ...
        for sentence in input_pack.get(Sentence):
            ...
            for rule in self.__rules:
                ...
                tagged_sentence = re.sub(
                    rule[3],
                    rule[2].strip() + reformat_rule + rule[2].strip(),
                    tagged_sentence,
                )

            entities = [
                em.text for em in input_pack.get(EntityMention, sentence)
            ]
            ...
            for entity in entities:
                ...
                # dont interfere with the regex execution.
